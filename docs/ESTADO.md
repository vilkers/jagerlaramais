...
| Fronteira da visão (v56.1) | **uma linha clara na aresta** entre casa vista e casa em névoa. `arestasDaNevoa(t)` calcula, `desenhaMapa` desenha (dois traços: vinco escuro embaixo, claro em cima). **A borda do tabuleiro não ganha linha** — ali a visão não acaba por névoa, acaba porque o mapa acaba. O poço é a exceção de sempre: nunca escurece, e sozinho na névoa ganha o próprio anel. HUD: `visão 62% · 44 na névoa`. 3 testes, verificados por mutação |
...
| Invisibilidade | **um herói** (Pombo Ciborgue). **Ward revela** — é a única fonte de visão que pega o invisível, e ela obedece à **regra do mato**: contra ele parado no mato só vale ward **plantada no mato**. Atacar entrega a posição. Revelado vence em qualquer lugar |
...
| **PvP em rede** (v56) | **Joga.** **Criar sala → Copiar convite → mandar.** Quem toca no link (`.../jogo/?sala=ABC123`) cai dentro; quem prefere digita o código de 6, que aparece grande na tela. **Sem campo de endereço e sem senha**: o jogo acha o servidor em `?srv=` → própria origem → `SALA_PADRAO` → o que ficou salvo, e no lugar da senha o servidor tem freio de tentativa (30 erros por IP em 10 min). ⚠️ **`SALA_PADRAO` nasce vazia** — alguém precisa publicar o servidor uma vez (`render.yaml` na raiz descreve o serviço) e colar a URL lá; até isso acontecer o online só funciona na mesma rede, e a tela explica. Servidor **autoritativo** — o estado mora nele e cada lado recebe só `estadoPara(lado)`. **14 ações roteadas**; o cliente manda intenção e nunca roda a regra. O modo é `modoRede`, ao lado de `aiMode`/`simMode`, e as duas portas de sempre resolvem: `ladoDaTela()` = meu lado, `mesaTravada()` += fora do meu turno. **Peça sem posição é regra agora**: o herói escondido chega com `pos:null`, e `em`/`visivelPara`/`naBase`/`rotaDaPos`/`seloVisao` guardam contra isso. **O cliente não calcula o próprio "ESCONDIDO"** — depende de onde o inimigo está, então vem carimbado em `despercebido`. Conferido com dois navegadores: névoa dinâmica, 0→1 inimigo ao entrar no campo de visão. **Falta:** reconexão, draft em rede, e **apertar o botão da hospedagem**. Ver `servidor/LEIA.md` |
...
node sim/rede.js          # 24 testes do servidor de salas, contra o servidor de verdade
